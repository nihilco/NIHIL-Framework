...
<section class="container site-content">
  <div class="row">
    <div class="col-sm-12">
      <h1>{{ $thread->title }}</h1>
      <h2><a href="#">{{ $thread->user->name }}</a></h2>
      {{ $thread->body }}
    </div>
  </div>
  <div class="row">
    ...
    <div class="col-sm-4">
    This thread was published {{ $thread->created_at->diffForHumans() }} by <a href="#">{{ $thread->user->name }}</a>, and currently has {{ $thread->replies_count }} {{ str_plural('replies', $thread->replies_count) }}.
    </div>
  </div>
</section>
