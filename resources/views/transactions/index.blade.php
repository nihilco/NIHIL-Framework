  ...
  <div class="row">
    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        <tr>
          <th scope="row"><a href="{{ $transaction->path() }}">{{ $transaction->type->name }}</a></th>
          <td>{{ $transaction->account->name }}</td>
          <td>{{ $transaction->from_source_id }}</td>
          <td>{{ $transaction->to_source_id }}</td>
    ...
    @else
    <p>No transactions at this time.</td></tr>
    @endif
    ...
    <div class="col">
      <div class="card">
            <div class="card-block">
    @if(!Auth::guest())
        <a href="/transactions/create" class="btn btn-lg btn-primary pull-right">New Transaction</a>
    @else
    Please <a href="{{ route('login') }}">login</a> to manage transactions.
    @endif    
